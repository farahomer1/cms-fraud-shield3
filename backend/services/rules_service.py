...
class RulesService:
    ...
    async def check_r01_quantity_cap(claim: dict) -> bool:
        ...
        # 3. Check rolling 30-day aggregate quantity billed across all NPIs
    ...
    async def check_r03_mbi_velocity(claim: dict) -> bool:
        ...
        ds = get_dataset()
        forty_eight_hours_ago = svc_date - timedelta(days=2)
        ...
        params = [
            bigquery.ScalarQueryParameter("mbi", "STRING", mbi),
            bigquery.ScalarQueryParameter("start_date", "DATE", forty_eight_hours_ago.isoformat()),
            bigquery.ScalarQueryParameter("end_date", "DATE", svc_date.isoformat()),
            bigquery.ScalarQueryParameter("claim_id", "STRING", claim.get("id") or ""),
        ]
    ...
    async def check_r04_pecos_change(claim: dict) -> bool:
        ...
        within 90 days of the current service date.
        """
        npi = claim.get("billing_npi") or claim.get("provider_id")
        if not npi:
            return False
    ...
    async def evaluate_claim(cls, claim: dict) -> dict:
        ...
        """
        # 1. Check if MBI is locked (active compromise lockdown)
        mbi = claim.get("mbi")
        is_mbi_locked = False
        if mbi:
            ds = get_dataset()
        ...
        # 3. Discrete scoring & prepay lifecycle
        if hit_count >= 2:
            score = 0.95
            status = "held"
            risk_level = "CRITICAL"
