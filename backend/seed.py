...
async def _seed_claims(bq: bigquery.Client) -> int:
    ...
    for r in rows:
        bq_rows.append({
            ...
            "normalized_data": json.dumps(r.get("normalized_data")) if r.get("normalized_data") is not None else None,
            "created_at": now,
            "updated_at": now,
        })
    if bq_rows:
...
async def reset_and_seed(bq: bigquery.Client) -> dict:
    ...
    from schema import create_dataset, create_tables
    logger.info("Ensuring dataset and tables exist in BigQuery...")
    await asyncio.to_thread(create_dataset, bq)
    await asyncio.to_thread(create_tables, bq)
