...
`analyze()` builds a JSON context (`=== CLAIM DATA ===`, `=== VETERAN RECORD ===`, `=== PROVIDER PROFILE ===`, `=== OTHER CLAIMS IN BATCH ===` — peers truncated to 10), calls `gemini_client.generate_json(prompt, system_prompt, temperature=0.3)` (system prompt loaded from `data/agent-prompts/<prompt_file>`, falling back to `_default_prompt()`), and returns a normalized finding:
...
│   ├── routes/                  # 13 feature routers → 51 endpoints
...
| Variable | Purpose |
|---|---|
| `VITE_API_BASE_URL` | Backend API base **including the `/api` suffix** — used by `apiClient.ts`, the SSE hook, and the PDF viewer. Unset in dev (falls back to `/api`, which the Vite proxy forwards to `:8000`); set to `https://<your-cloud-run-url>/api` for production builds. |
...
> ⚠️ `.env.example` lists `VITE_API_URL`, but **nothing in `src/` reads it** — the variable the code actually uses is `VITE_API_BASE_URL`. Treat `VITE_API_URL` as vestigial.
...
51 endpoints across 13 routers. Full interactive documentation: **`http://localhost:8000/docs`** (Swagger) / **`/redoc`**.
...
VITE_API_BASE_URL=https://pivot-backend-<HASH>-<REGION>.a.run.app/api
EOF
