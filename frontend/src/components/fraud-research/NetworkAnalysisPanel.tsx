...
interface FraudRing {
  ...
  members: Array<{
    name: string;
    type: 'provider' | "member" | 'representative';
    role: string;
    connections: number;
  }>;
  total_claims: number;
...
const fraudRings: FraudRing[] = [
  {
    ...
    risk_score: 94,
    members: [
      { name: 'Tampa Bay Members Services', type: 'provider', role: 'Hub Provider', connections: 12 },
      ...
      { name: '14 shared members', type: "member", role: 'Shared Claimants', connections: 14 },
    ],
    total_claims: 287,
    total_amount: 842000,
    shared_beneficiaries: 14,
    pattern_description: 'Three providers sharing the same member pool and claims representative. Coordinated billing patterns with staggered submission dates. Same diagnosis codes used across providers.',
    confidence: 0.91,
  },
  {
    id: 'RING-002',
    ...
    risk_score: 89,
    members: [
      { name: 'Orlando Member Care LLC', type: 'provider', role: 'Primary Provider', connections: 9 },
      { name: 'Central FL Benefits Corp', type: 'provider', role: 'Clinical Assessment Provider', connections: 7 },
      { name: 'R. Thompson (Rep)', type: 'representative', role: 'Claims Agent', connections: 22 },
      { name: '22 shared members', type: "member", role: 'Recruited Claimants', connections: 22 },
    ],
    total_claims: 198,
    total_amount: 634000,
    shared_beneficiaries: 22,
    pattern_description: 'Single claims agent recruiting beneficiaries and routing through two coordinated providers. Clinical assessment forms show template-like similarity (98.7% structural match). Member addresses cluster in 3-mile radius.',
    confidence: 0.87,
  },
  {
    id: 'RING-003',
    ...
    risk_score: 82,
    members: [
      { name: 'Pensacola Members Aid Group', type: 'provider', role: 'Lead Provider', connections: 6 },
      ...
      { name: '11 shared members', type: "member", role: 'Pension Claimants', connections: 11 },
    ],
    total_claims: 145,
    total_amount: 398000,
    shared_beneficiaries: 11,
    pattern_description: 'Providers targeting elderly members for beneficiary exploitation. Claims submitted within days of initial contact. Billing amounts consistently at maximum allowed thresholds.',
    confidence: 0.83,
  },
];

// --- Connection heatmap data (provider-to-provider shared members) ---
const providers = [
  'Tampa Bay VS',
  'Bay Area CG',
  'Coastal Ben.',
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        <Stack direction="row" spacing={1} alignItems="flex-start">
          ...
          <Box>
            ...
            <Typography variant="body2" sx={{ color: '#5B616B', mt: 0.5, lineHeight: 1.6 }}>
              ...
              beneficiaries with suspicious interconnections. Community detection algorithms (Louvain, Label Propagation) find
      ...
      <Stack direction="row" spacing={2} flexWrap="wrap">
        ...
        <Chip
          icon={<GroupWorkIcon />}
          label={`${fraudRings.reduce((sum, r) => sum + r.shared_beneficiaries, 0)} Shared Members`}
      ...
      {viewMode === 'connections' && (
        <>
          ...
          <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 2 }}>
            <Typography variant="subtitle2" fontWeight={600} sx={{ color: '#112E51', mb: 0.5 }}>
              Provider-to-Provider Shared Member Matrix
            </Typography>
            <Typography variant="caption" sx={{ color: '#5B616B', display: 'block', mb: 2 }}>
              Numbers indicate shared members between providers. High values suggest coordinated fraud activity.
            </Typography>
            <TableContainer>
              <Table size="small">
                <TableHead>
