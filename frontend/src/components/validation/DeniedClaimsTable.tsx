...
function fallbackAnalystName(claimId: string): string {
  let hash = 0;
  for (let i = 0; i < claimId.length; i++) {
    hash = (hash * 31 + claimId.charCodeAt(i)) | 0;
  }
  return FALLBACK_ANALYST_NAMES[Math.abs(hash) % FALLBACK_ANALYST_NAMES.length];
}

function getDeniedBy(claim: Claim): string {
  return claim.decision?.actor || fallbackAnalystName(claim.id);
}
